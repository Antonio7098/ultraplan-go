...
Repair projections retain `consumed` counters and a bounded `runtime` observation with provider/model/session identity, timings, usage, cache, estimated cost, event count, and observed tool-call count. `cycles` contains only immutable completed cycles and exposes their scope, gate durations/output bytes, and cleanup record. `repair_result` carries the terminal semantic result. A blocked, failed, escalated, or stalled semantic outcome also fails the durable operation; only `verified` and `verified_with_findings` are operational successes.
