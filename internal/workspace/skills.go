...
Follow progress, inspect partial failures, and continue until the execution state is complete or a genuine blocker requires the user. Do not replace this governed execution with untracked ad-hoc implementation.`,
		},
		{
			...
			PromptAvailable:  true,
			StageWorkflow: `Preview the review scope first:
...
func renderStageSkill(skill StageSkill) string {
   ...
   The resolved prompt can include workspace or project overrides and therefore takes precedence over the canonical prompt below.`, skill.Stage)
	...
	return fmt.Sprintf(`---
name: %s
description: Manually run the UltraPlan %s stage for a selected project sprint. Use only when the user explicitly invokes $%s or directly asks to run this exact UltraPlan stage; do not invoke implicitly.
---
...
9. Run `+"`ultraplan sprint <project> <sprint> validate %s`"+` when supported. Fix validation findings within this stage rather than declaring success early.
